...
export default function PropAll(prop) {
    const listItems = props.filter((element) => {
        if (prop.inputName === '' && prop.inputTag === 'All') {
            ...
            return element.name.toLowerCase().startsWith(prop.inputName.toLowerCase())
        }
        else {
            return element.name.toLowerCase().startsWith(prop.inputName.toLowerCase()) && element.tag.toLowerCase().includes(prop.inputTag.toLowerCase())
        }
    });
